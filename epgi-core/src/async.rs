...
pub(super) struct AsyncWorkQueue<E: Element> {
    pub(super) inner: Option<Box<AsyncWorkQueueInner<E>>>,
}

pub(super) struct AsyncWorkQueueInner<E: Element> {
    pub(super) current: Option<AsyncQueueCurrentEntry<E>>,
    pub(super) backqueue: Vec<AsyncQueueBackqueueEntry<E::ArcWidget>>,
}

pub(super) struct AsyncQueueCurrentEntry<E: Element> {
    pub(super) work: Work<E::ArcWidget>,
    pub(super) stash: AsyncStash<E>,
}
...
pub(crate) struct AsyncStash<E: Element> {
    ...
    pub(super) output: AsyncOutput<E>,
}

pub(super) struct AsyncQueueBackqueueEntry<ArcWidget> {
    pub(super) work: Work<ArcWidget>,
    ...
    E: Element,
{
    ...
    pub(super) fn current(&self) -> Option<&AsyncQueueCurrentEntry<E>> {
        self.inner.as_ref().and_then(|inner| inner.current.as_ref())
    }
    pub(super) fn current_mut(&mut self) -> Option<&mut AsyncQueueCurrentEntry<E>> {
        self.inner.as_mut().and_then(|inner| inner.current.as_mut())
    }

    pub(super) fn remove_current(&mut self) -> Option<AsyncQueueCurrentEntry<E>> {
        self.inner
            .as_mut()
            .and_then(|inner| (&mut inner.current).take()) // rust-analyzer#14933
    }

    pub(super) fn push_backqueue(&mut self, work: Work<E::ArcWidget>, barrier: CommitBarrier) {
    ...
    pub(super) fn try_remove(&mut self, lane_pos: LanePos) -> AsyncDequeueResult<E> {
        let Some(inner) = &mut self.inner else {return AsyncDequeueResult::NotFound};
...
pub(super) enum AsyncDequeueResult<E: Element> {
...
pub(super) enum AsyncOutput<E: Element> {
    Uninitiated {
        barrier: CommitBarrier,
    },
    Suspended {
